...
import wholegraph_torch as wgth
...
def train(train_data, valid_data, model, optimizer, wg_optimizer, global_comm):
    if wgth.get_rank() == 0:
...
def main_func():
    ...
    global_comm, local_comm = wgth.init_torch_env_and_create_wg_comm(
        wgth.get_rank(),
        wgth.get_world_size(),
        wgth.get_local_rank(),
        wgth.get_local_size(),
    ...
    graph_structure_wholegraph_type = "continuous"
    graph_structure_wholegraph_location = "cuda"
    ...
    csr_row_ptr_wg_tensor = wgth.create_wholegraph_tensor_from_filelist(
        graph_comm,
        graph_structure_wholegraph_type,
        graph_structure_wholegraph_location,
        os.path.join(args.root_dir, "homograph_csr_row_ptr"),
        torch.int64,
    )
    csr_col_ind_wg_tensor = wgth.create_wholegraph_tensor_from_filelist(
        graph_comm,
        graph_structure_wholegraph_type,
        graph_structure_wholegraph_location,
    ...
    graph_structure.set_csr_graph(csr_row_ptr_wg_tensor, csr_col_ind_wg_tensor)

    feature_comm = global_comm if args.use_global_embedding else local_comm

    embedding_wholegraph_type = args.embedding_memory_type
    embedding_wholegraph_location = (
    ...
    if wgth.get_rank() == 0:
        print(
            f"graph_structure: type={graph_structure_wholegraph_type}, "
            f"location={graph_structure_wholegraph_location}\n"
            f"embedding: type={embedding_wholegraph_type}, "
            f"location={embedding_wholegraph_location}, "
    ...
    if args.train_embedding is False:
        node_feat_wg_embedding = wgth.create_embedding_from_filelist(
            feature_comm,
            embedding_wholegraph_type,
            embedding_wholegraph_location,
        ...
        node_feat_wg_embedding = wgth.create_embedding(
            feature_comm,
            embedding_wholegraph_type,
            embedding_wholegraph_location,
            embedding_dtype,
        ...
        wg_optimizer = wgth.create_wholegraph_optimizer(
            node_feat_wg_embedding, "adam", {}
        )
    wgth.set_framework(args.framework)
    model = wgth.HomoGNNModel(graph_structure, node_feat_wg_embedding, args)
    model.cuda()
    model = DDP(model, delay_allreduce=True)
    optimizer = apex.optimizers.FusedAdam(model.parameters(), lr=args.lr)

    train(train_ds, valid_ds, model, optimizer, wg_optimizer, global_comm)
    test(test_ds, model)
