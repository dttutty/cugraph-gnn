# SPDX-FileCopyrightText: Copyright (c) 2019-2025, NVIDIA CORPORATION.
# SPDX-License-Identifier: Apache-2.0

from wholegraph_torch._version import __git_commit__, __version__

from .comm import (
    WholeGraphCommunicator,
    create_group_communicator,
    destroy_communicator,
)
from .comm import (
    get_global_communicator,
    get_local_node_communicator,
    get_local_device_communicator,
    split_communicator,
    get_local_mnnvl_communicator,
)

from .embedding import (
    WholeGraphOptimizer,
    create_wholegraph_optimizer,
    destroy_wholegraph_optimizer,
)
from .embedding import (
    WholeGraphCachePolicy,
    create_builtin_cache_policy,
    create_wholegraph_cache_policy,
    destroy_wholegraph_cache_policy,
)
from .embedding import (
    WholeGraphEmbedding,
    create_embedding,
    create_embedding_from_filelist,
    destroy_embedding,
)
from .embedding import WholeGraphEmbeddingModule

from .initialize import (
    init,
    init_torch_env,
    init_torch_env_and_create_wg_comm,
    finalize,
)

from .tensor import (
    WholeGraphTensor,
    create_wholegraph_tensor,
    create_wholegraph_tensor_from_filelist,
    destroy_wholegraph_tensor,
)
from .distributed_tensor import DistTensor, DistEmbedding
from .distributed_matrix import DistMatrix
from .distributed_tensor_utils import empty, is_empty
from .storage_backend import (
    StorageBackend,
    StoragePolicy,
    WholeGraphLocation,
    WholeGraphMemoryType,
    backend_to_wholegraph_memory_type,
    has_mapped_storage_support,
    infer_default_backend,
    resolve_storage_policy,
)
from .graph_structure import GraphStructure

from .utils import get_part_file_name, get_part_file_list

from .distributed_launch import add_distributed_launch_options, distributed_launch
from .distributed_launch import get_rank, get_world_size, get_local_rank, get_local_size

from .common_options import (
    add_common_graph_options,
    add_common_model_options,
    add_common_sampler_options,
)
from .common_options import (
    add_training_options,
    add_dataloader_options,
    add_node_classfication_options,
)

from .gnn_model import set_framework, create_gnn_layers, create_sub_graph, HomoGNNModel
from .data_loader import (
    create_node_classification_datasets,
    get_train_dataloader,
    get_valid_test_dataloader,
)
from .wholegraph_env import compile_cpp_extension
