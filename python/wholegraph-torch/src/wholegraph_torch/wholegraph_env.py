...
from .utils import wholegraph_dtype_to_torch_dtype, torch_dtype_to_wholegraph_dtype
...
def torch_malloc_env_fn(
    ...
    pinned = False
    device = None
    if malloc_type_int == int(wgb.WholeGraphMemoryAllocType.MatDevice):
        device = torch.device("cuda")
    elif malloc_type_int == int(wgb.WholeGraphMemoryAllocType.MatHost):
        device = torch.device("cpu")
    else:
        assert malloc_type_int == int(wgb.WholeGraphMemoryAllocType.MatPinned)
        device = torch.device("cpu")
        pinned = True
    dtype = wholegraph_dtype_to_torch_dtype(wgb.WholeGraphDataType(dtype_int))
...
def wrap_torch_tensor(t: Union["torch.Tensor", None]) -> wgb.WrappedLocalTensor:
    py_desc = wgb.PyWholeGraphTensorDescription()
    wg_t = wgb.WrappedLocalTensor()
    if t is None:
        return wg_t.wrap_tensor(py_desc, 0)
    py_desc.set_dtype(torch_dtype_to_wholegraph_dtype(t.dtype))
    py_desc.set_storage_offset(0)
    py_desc.set_shape(tuple(t.shape))
    py_desc.set_stride(tuple(t.stride()))
    return wg_t.wrap_tensor(py_desc, t.data_ptr())
...
def compile_cpp_extension():
    ...
    torch_utils_cpp_ext.load(
        name="wholegraph_torch.wholegraph_torch_ext",
        sources=[
    ...
    torch_cpp_ext_lib = importlib.import_module("wholegraph_torch.wholegraph_torch_ext")
