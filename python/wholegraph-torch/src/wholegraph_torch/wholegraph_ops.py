...
def unweighted_sample_without_replacement(
    wg_csr_row_ptr_tensor: wgb.PyWholeGraphTensor,
    wg_csr_col_ptr_tensor: wgb.PyWholeGraphTensor,
    ...
    """
    assert wg_csr_row_ptr_tensor.dim() == 1
    assert wg_csr_col_ptr_tensor.dim() == 1
    ...
    wgb.csr_unweighted_sample_without_replacement(
        wg_csr_row_ptr_tensor,
...
def weighted_sample_without_replacement(
    wg_csr_row_ptr_tensor: wgb.PyWholeGraphTensor,
    wg_csr_col_ptr_tensor: wgb.PyWholeGraphTensor,
    wg_csr_weight_ptr_tensor: wgb.PyWholeGraphTensor,
    center_nodes_tensor: "torch.Tensor",
    max_sample_count: int,
    random_seed: Union[int, None] = None,
    need_center_local_output: bool = False,
    ...
    assert wg_csr_row_ptr_tensor.dim() == 1
    assert wg_csr_col_ptr_tensor.dim() == 1
    assert wg_csr_weight_ptr_tensor.dim() == 1
    assert wg_csr_weight_ptr_tensor.shape[0] == wg_csr_col_ptr_tensor.shape[0]
    assert center_nodes_tensor.dim() == 1
    if random_seed is None:
        random_seed = random.getrandbits(64)
    output_sample_offset_tensor = torch.empty(
    ...
    if need_edge_output:
        output_edge_gid_context = TorchMemoryContext()
        output_edge_gid_c_context = output_edge_gid_context.get_c_context()
    wgb.csr_weighted_sample_without_replacement(
        wg_csr_row_ptr_tensor,
        wg_csr_col_ptr_tensor,
...
def generate_random_positive_int_cpu(
    ...
    wgb.host_generate_random_positive_int(
...
def generate_exponential_distribution_negative_float_cpu(
    ...
    wgb.host_generate_exponential_distribution_negative_float(
        random_seed, sub_sequence, wrap_torch_tensor(output)
    )
    return output
