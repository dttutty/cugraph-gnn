...
from .distributed_tensor_utils import (
    copy_host_global_tensor_to_local,
    create_wg_dist_tensor,
    create_wg_dist_tensor_from_files,
)
from .embedding import WholeGraphCachePolicy, WholeGraphEmbedding
...
class DistTensor:
    ...
    backend : Literal["vmm", "nccl"] = "nccl"
        The storage backend. Default is "nccl".
    """
        ...
        **kwargs,
        ...
        if src is None:
            ...
            if isinstance(src, list):
                ...
                # Only support the binary file format directly loaded via WholeGraph API for now
    ...
    def load_from_global_tensor(self, tensor):
        ...
        if isinstance(self._tensor, WholeGraphEmbedding):
...
class DistEmbedding(DistTensor):
    ...
    backend : Literal["vmm", "nccl"] = "nccl"
        The storage backend. Default is "nccl".
    cache_policy : Optional[WholeGraphCachePolicy] = None
