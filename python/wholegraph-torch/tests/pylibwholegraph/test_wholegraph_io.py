...
def load_routine_func(
    ...
    data_type = wgb.WholeGraphDataType.DtInt
    ...
    if entry_partition is None:
        per_rank_entry = wgb.equal_partition_plan(extra_embedding_count, world_size)
    ...
    for mt in [
        ...
        for ml in [
            ...
            if not wg_comm.support_type_location(mt, ml):
                continue
            wholegraph_root_tensor = wgb.create_wholegraph_matrix(
                data_type,
                extra_embedding_count,
            ...
            wholegraph_tensor = wholegraph_root_tensor.get_sub_tensor(
                [-1, storage_offset], [-1, -1]
            )
            wholegraph_tensor.from_filelist(file_list, round_robin_size)
            local_tensor, local_offset = wholegraph_tensor.get_local_tensor(
            ...
            del wholegraph_tensor
            wgb.destroy_wholegraph_tensor(wholegraph_root_tensor)
...
def test_wholegraph_load(
    ...
    if round_robin_size != 0:
        entry_per_rank = wgb.equal_partition_plan(extra_embedding_count, gpu_count)
...
def store_routine_func(
    ...
    mt = wgb.WholeGraphMemoryType.MtContinuous
    ml = wgb.WholeGraphMemoryLocation.MlHost
    ...
    local_root_tensor, local_root_offset = wholegraph_root_tensor.get_local_tensor(
        torch_import_from_dlpack, wgb.WholeGraphMemoryLocation.MlHost, world_rank
    )
    ...
    wholegraph_tensor.to_file(filename)
