...
def copy_host_1D_tensor_to_wholegraph(
    wg_array, host_tensor, world_rank, world_size, wg_comm
):
    local_tensor_cuda, local_start = wg_array.get_local_tensor(
        torch_import_from_dlpack, wgb.WholeGraphMemoryLocation.MlDevice, world_rank
    )
    assert local_tensor_cuda.dim() == 1
    local_count = wg_array.get_local_entry_count()
    local_start_ref = wg_array.get_local_entry_start()
...
def int_to_wholegraph_datatype(value: int):
    if value == 0:
        return wgb.WholeGraphDataType.DtInt
    if value == 1:
        return wgb.WholeGraphDataType.DtInt64
    if value == 2:
        return wgb.WholeGraphDataType.DtFloat
    if value == 3:
        return wgb.WholeGraphDataType.DtDouble
    else:
        raise ValueError("invalid int_to_wholegraph_datatype value")


def int_to_wholegraph_location(value: int):
    if value == 0:
        return wgb.WholeGraphMemoryLocation.MlHost
    if value == 1:
        return wgb.WholeGraphMemoryLocation.MlDevice
    else:
        raise ValueError("invalid int_to_wholegraph_location value")


def int_to_wholegraph_type(value: int):
    if value == 0:
        return wgb.WholeGraphMemoryType.MtContinuous
    if value == 1:
        return wgb.WholeGraphMemoryType.MtDistributed
    else:
        raise ValueError("invalid int_to_wholegraph_type value")
