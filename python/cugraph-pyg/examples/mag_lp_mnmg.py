    ...
    def __init__(
        ...
        if self.learn_embeddings:
            ...
            for node_type in sorted(num_nodes.keys()):
                ...
                self.embeddings[node_type] = wgth.embedding.WholeGraphEmbeddingModule(
                    wg_node_emb
                )
        else:
            self.mp = Sequential(
...
def init_pytorch_worker(global_rank, local_rank, world_size, cugraph_id):
    ...
    from wholegraph_torch.initialize import init as wg_init
...
if __name__ == "__main__":
    ...
    vtypes = sorted(num_nodes.keys())
    print("initializing node prior features...")
    for vtype in vtypes:
        if global_rank == 0:
            feature_store[vtype, "node_prior", None] = torch.ones(
                (int(num_nodes[vtype]),), device="cuda", dtype=dtype
            )
        else:
            feature_store[vtype, "node_prior", None] = empty(dim=1)

    print("updating edge features from node priors...")
    for etype in graph_store.get_all_edge_attrs():
        ...
        feature_store[etype.edge_type, "x", None] = (
            feature_store[src_type, "node_prior", None][src]
            + feature_store[dst_type, "node_prior", None][dst]
    ...
    if args.learn_embeddings:
        wg_optimizer = wgth.create_wholegraph_optimizer(
            [
                model.embeddings[node_type].wg_embedding
                for node_type in sorted(num_nodes.keys())
            ],
            "adam",
            {},
    ...
    if args.learn_embeddings:
        local_x1 = (
            ...
            .wg_embedding.get_embedding_tensor()
            .get_local_tensor()[0]
        )
    else:
    ...
    torch.distributed.barrier()
    from wholegraph_torch.initialize import finalize as wg_finalize

    wg_finalize()  # will also destroy the process group
