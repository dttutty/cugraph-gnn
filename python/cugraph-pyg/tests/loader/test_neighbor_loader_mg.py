...
wholegraph_torch = import_optional("wholegraph_torch")

pytestmark = pytest.mark.skipif(
    isinstance(wholegraph_torch, MissingModule),
    reason="wholegraph_torch not available",
)


def cleanup_pytorch_worker():
    with suppress(Exception):
        cugraph_comms_shutdown()

    with suppress(Exception):
        wholegraph_torch.initialize.finalize()
...
def run_test_neighbor_loader_mg(rank, uid, world_size, specify_size):
    ...
    try:
        ...
        sz = (KARATE_NUM_NODES, KARATE_NUM_NODES) if specify_size else None
        graph_store = GraphStore()
        graph_store.put_edge_index(ei, ("person", "knows", "person"), "coo", False, sz)

        feature_store = FeatureStore()
        feature_store["person", "feat", None] = torch.randint(
            128, (KARATE_NUM_NODES, 16)
        )

        ix_train = torch.tensor_split(
            torch.arange(KARATE_NUM_NODES), world_size, axis=0
        )[rank]
        ...
        for batch in loader:
            ...
            assert (
                feature_store["person", "feat", None][batch.n_id] == batch.feat
            ).all()
    finally:
        cleanup_pytorch_worker()
