...
#include "../../tests/wholegraph/wholegraph_test_utils.hpp"
...
typedef struct GatherScatterBenchParam {
  ...
  wholegraph_memory_type_t get_memory_type() const { return memory_type; }

  wholegraph_memory_location_t get_memory_location() const { return memory_location; }
  ...
  wholegraph_dtype_t get_embedding_type() const { return embedding_type; }
  ...
  GatherScatterBenchParam& set_memory_type(wholegraph_memory_type_t new_memory_type)
  {
    memory_type = new_memory_type;
    return *this;
  }
  GatherScatterBenchParam& set_memory_location(wholegraph_memory_location_t new_memory_location)
  {
  ...
  {
    int64_t entry_size   = wholegraph_dtype_get_element_size(embedding_type) * get_embedding_dim();
  ...
  GatherScatterBenchParam& set_embedding_dim(int64_t new_embedding_dim)
  {
    ...
    int64_t entry_size   = wholegraph_dtype_get_element_size(embedding_type) * embedding_dim;
 ...
 private:
  ...
  {
    return embedding_table_size / wholegraph_dtype_get_element_size(embedding_type) /
           embedding_dim;
  }
  int64_t get_indices_count() const
  {
    return gather_size / wholegraph_dtype_get_element_size(embedding_type) / embedding_dim;
  }
  ...
  GatherScatterBenchParam& set_embedding_type(wholegraph_dtype_t new_embedding_type)
  {
    embedding_type = new_embedding_type;
    return *this;
  }
  GatherScatterBenchParam& set_indices_type(wholegraph_dtype_t new_indices_type)
  {
    indices_type = new_indices_type;
    return *this;
  }
  GatherScatterBenchParam& set_output_type(wholegraph_dtype_t new_output_type)
  {
  ...
  wholegraph_dtype_t indices_type   = WHOLEGRAPH_DT_INT64;
...
std::string get_memory_type_string(wholegraph_memory_type_t memory_type)
{
  std::string str;
  switch (memory_type) {
    case WHOLEGRAPH_MT_NONE: str = "WHOLEGRAPH_MT_NONE"; break;
    case WHOLEGRAPH_MT_CONTINUOUS: str = "WHOLEGRAPH_MT_CONTINUOUS"; break;
    case WHOLEGRAPH_MT_DISTRIBUTED: str = "WHOLEGRAPH_MT_DISTRIBUTED"; break;
    default: break;
  }
  return str;
}

std::string get_memory_location_string(wholegraph_memory_location_t memory_location)
{
  std::string str;
  switch (memory_location) {
    case WHOLEGRAPH_ML_NONE: str = "WHOLEGRAPH_ML_NONE"; break;
    case WHOLEGRAPH_ML_DEVICE: str = "WHOLEGRAPH_ML_DEVICE"; break;
    case WHOLEGRAPH_ML_HOST: str = "WHOLEGRAPH_ML_HOST"; break;
    default: break;
  }
  return str;
}
...
void gather_scatter_benchmark(GatherScatterBenchParam& params)
{
  ...
  WHOLEGRAPH_CHECK_NOTHROW(g_dev_count >= 1);
  if (params.get_num_gpu() == 0) { params.set_num_gpu(g_dev_count); }
  MultiProcessRun(
    g_dev_count,
    [&params](int local_rank, int local_size) {
      WHOLEGRAPH_CHECK_NOTHROW(wholegraph_init(0) == WHOLEGRAPH_SUCCESS);
      WG_CUDA_CHECK_NO_THROW(cudaSetDevice(local_rank));
      ...
      WHOLEGRAPH_CHECK_NOTHROW(wholegraph_create_tensor(&embedding_tensor,
                                                          ...
                                                          params.get_partition_method() == 1
                                                            ? rank_partition.data()
                                                            : nullptr) == WHOLEGRAPH_SUCCESS);

      cudaStream_t stream;
      WG_CUDA_CHECK_NO_THROW(cudaStreamCreate(&stream));
      ...
      WG_CUDA_CHECK_NO_THROW(cudaMallocHost(&host_indices, indices_buffer_size));
      WG_CUDA_CHECK_NO_THROW(cudaMalloc(&dev_indices, indices_buffer_size));
      WG_CUDA_CHECK_NO_THROW(cudaMalloc(&dev_gather_buffer, gather_buffer_size));

      wholegraph::bench::host_random_init_integer_indices(
        host_indices, indices_desc, embedding_desc.sizes[0]);
      WG_CUDA_CHECK_NO_THROW(cudaMemcpyAsync(dev_indices,
                                             host_indices,
                                             wholegraph_get_memory_size_from_array(&indices_desc),
                                             cudaMemcpyHostToDevice,
                                             stream));
      WG_CUDA_CHECK_NO_THROW(cudaStreamSynchronize(stream));
      WHOLEGRAPH_CHECK_NOTHROW(wholegraph_communicator_barrier(wg_comm) == WHOLEGRAPH_SUCCESS);
      ...
      WHOLEGRAPH_CHECK_NOTHROW(
        ...
        WHOLEGRAPH_SUCCESS);
      WHOLEGRAPH_CHECK_NOTHROW(wholegraph_make_tensor_from_pointer(
                                  &output_tensor, dev_gather_buffer, &output_tensor_desc) ==
                                WHOLEGRAPH_SUCCESS);
      WG_CUDA_CHECK_NO_THROW(cudaStreamSynchronize(stream));
      WHOLEGRAPH_CHECK_NOTHROW(wholegraph_communicator_barrier(wg_comm) == WHOLEGRAPH_SUCCESS);

      const auto barrier_fn = [&wg_comm]() -> void {
        WHOLEGRAPH_CHECK_NOTHROW(wholegraph_communicator_barrier(wg_comm) == WHOLEGRAPH_SUCCESS);
      };
      ...
      if (local_rank == 0) {
        printf(
          ...
          wholegraph_dtype_get_element_size(params.get_embedding_type()),
      ...
      if (test_type.compare("gather") == 0) {
        MultiProcessMeasurePerformance(
          [&] {
            wholegraph_gather(embedding_tensor,
                               indices_tensor,
                               output_tensor,
                               wholegraph::get_cached_env_func(),
                               stream);
          },
          wg_comm,
          meter,
        ...
        MultiProcessMeasurePerformance(
          [&] {
            wholegraph_scatter(output_tensor,
                                indices_tensor,
                                embedding_tensor,
                                wholegraph::get_cached_env_func(),
                                stream);
      ...
      wholegraph::drop_cached_env_func_cache();

      WHOLEGRAPH_CHECK_NOTHROW(wholegraph_destroy_tensor(indices_tensor) == WHOLEGRAPH_SUCCESS);
      WHOLEGRAPH_CHECK_NOTHROW(wholegraph_destroy_tensor(output_tensor) == WHOLEGRAPH_SUCCESS);

      WG_CUDA_CHECK_NO_THROW(cudaFreeHost(host_indices));
      WG_CUDA_CHECK_NO_THROW(cudaFree(dev_indices));
      WG_CUDA_CHECK_NO_THROW(cudaFree(dev_gather_buffer));

      WHOLEGRAPH_CHECK_NOTHROW(wholegraph_destroy_tensor(embedding_tensor) ==
                                WHOLEGRAPH_SUCCESS);

      WHOLEGRAPH_CHECK_NOTHROW(wholegraph::destroy_all_communicators() == WHOLEGRAPH_SUCCESS);

      WHOLEGRAPH_CHECK_NOTHROW(wholegraph_finalize() == WHOLEGRAPH_SUCCESS);
    },
    true);
}
...
int main(int argc, char** argv)
{
  ...
  struct option opts[] = {
    ...
    {"memory_type",
     required_argument,
     NULL,
	     't'},  // 0: None, 1: Continuous, 2: Distributed
  ...
  const char* usage =
    ...
    "  -t, --memory_type   specify wholegraph type, 0: None, 1: Continuous, 2: Distributed\n"
    "  -l, --memory_location    specify wholegraph location, 0: None, 1: Device, 2: Host\n"
    ...
    "  -b, --distributed_backend   specify distributed backend: nccl\n";
  ...
  while ((c = getopt_long(argc, argv, optstr, opts, NULL)) != -1) {
    ...
    switch (c) {
      ...
      case 't':
        ...
        params.set_memory_type(static_cast<wholegraph_memory_type_t>(val));
        break;
      case 'l':
        val = strtol(optarg, &endptr, 10);
        if (*endptr != '\0' || val < 0 || val > 2) {
        ...
        params.set_memory_location(static_cast<wholegraph_memory_location_t>(val));
        break;
      case 'e':
