...
/**
 * Determine alignment of a WholeGraph matrix, in element count, maximum 16 / element_size.
 * @param embedding_desc : wholegraph_matrix_description_t matrix description.
 * @return : Alignment that can be used, in element count.
 */
inline int determine_wholegraph_alignment_elt_count(
  wholegraph_matrix_description_t embedding_desc)
{
  int elt_size = static_cast<int>(wholegraph_dtype_get_element_size(embedding_desc.dtype));
  WHOLEGRAPH_CHECK(elt_size != -1);
 ...
 * @param memory_desc : wholegraph_matrix_description_t matrix description.
 * @return : Alignment that can be used, in element count.
 */
inline int determine_memory_alignment_elt_count(const void* ptr,
                                                wholegraph_matrix_description_t memory_desc)
{
  int elt_size = static_cast<int>(wholegraph_dtype_get_element_size(memory_desc.dtype));
  WHOLEGRAPH_CHECK(elt_size != -1);
  int alignment   = 16 / elt_size;
  int64_t int_ptr = reinterpret_cast<int64_t>(ptr);
  WHOLEGRAPH_CHECK(int_ptr % elt_size == 0);
  int_ptr /= elt_size;
...
__global__ void gather_func_kernel(wholegraph_gref_t embedding_gref,
                                   wholegraph_matrix_description_t embedding_desc,
                                   ...
                                   wholegraph_matrix_description_t output_desc)
{
  ...
  wholegraph::device_reference<EmbeddingT> embedding_dev_ref(embedding_gref);
...
template <typename EmbeddingT,
          ...
          int SUB_WARP_SIZE = 1,
          int ALIGNMENT     = 1>
__global__ void gather_func_sub_warp_kernel(wholegraph_gref_t embedding_gref,
                                            wholegraph_matrix_description_t embedding_desc,
                                            const IndexT* indices,
                                            int64_t indice_count,
                                            bool gather_with_sorted_ids,
...
void gather_temp_func(wholegraph_gref_t embedding_gref,
                      wholegraph_matrix_description_t embedding_desc,
                      void* indices,
                      int64_t indice_count,
                      bool gather_with_sorted_ids,
                      ...
                      int gather_sms)
{
  WHOLEGRAPH_EXPECTS(output_desc.sizes[0] == indice_count,
                      "gather_func, output shape[0]=%ld, but indice_count=%ld",
                      output_desc.sizes[0],
                      indice_count);
  if (indice_count == 0 || embedding_desc.sizes[1] == 0) return;
  int wg_alignment = determine_wholegraph_alignment_elt_count(embedding_desc);
  int mm_alignment = determine_memory_alignment_elt_count(output, output_desc);
  int alignment    = std::min<int>(wg_alignment, mm_alignment);
  // int embedding_size = embedding_desc.sizes[1];
  // int thread_num       = wholegraph::div_rounding_up_safe<int>(embedding_size, alignment);
  // thread_num           = std::min(thread_num, 512);
  // int64_t block_count = indice_count >= 1024 ? 1024 : static_cast<int>(indice_count);

  void (*kernel_fn)(wholegraph_gref_t,
                    ...
                    const IndexT*,
                    OutputT*,
                    wholegraph_matrix_description_t) = nullptr;
  ...
  switch (alignment) {
    ...
    default: {
      WHOLEGRAPH_FAIL("gather func alignment=%d.", alignment);
      return;
    }
  }
  int block_size  = 1024;
...
#undef SWITCH_GATHER_FUNC_WITH_ALIGNMENT
    ...
    int sm_count  = 100;
    int device_id = 0;
    WG_CUDA_CHECK(cudaGetDevice(&device_id));
    WG_CUDA_CHECK(cudaDeviceGetAttribute(&sm_count, cudaDevAttrMultiProcessorCount, device_id));
...
void scatter_temp_func(const void* input,
                       ...
                       int scatter_sms)
{
  WHOLEGRAPH_EXPECTS(input_desc.sizes[0] == indice_count,
  ...
  switch (alignment) {
    ...
    default: {
      WHOLEGRAPH_FAIL("scatter func alignment=%d.", alignment);
      return;
    }
  }
  int block_size  = 256;
