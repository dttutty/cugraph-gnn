                              ...
                              cudaStream_t stream)
{
  ...
  cache_set_lid_handle.device_malloc(indices_num_run, WHOLEGRAPH_DT_INT);
  int* cache_set_lid    = static_cast<int*>(cache_set_lid_handle.pointer());
  int const block_count = wholegraph::div_rounding_up_unsafe(indices_num_run, 32);
  ...
  unique_cache_set_lid_handle->device_malloc(indices_num_run, WHOLEGRAPH_DT_INT);
  unique_cache_set_count_handle->device_malloc(indices_num_run, WHOLEGRAPH_DT_INT);
  unique_cache_set_start_handle->device_malloc(indices_num_run, WHOLEGRAPH_DT_INT);
  cache_set_num_run_handle.device_malloc(1, WHOLEGRAPH_DT_INT);
...
__global__ void UpdateCacheDirectKernel(const int* unique_cache_set_lid,
                                        ...
                                        int cache_set_coverage)
{
  static_assert(wholegraph::embedding_cache_base::kCacheSetSize == 32);
  int64_t const cache_set_lid = unique_cache_set_lid[blockIdx.x];
  local_cache_line_tag += cache_set_lid * wholegraph::embedding_cache_base::kCacheSetSize;
  local_cache_line_lfu_count += cache_set_lid * wholegraph::embedding_cache_base::kCacheSetSize;
  local_access_count += cache_set_lid * cache_set_coverage;
  local_cached_data +=
    cache_set_lid * wholegraph::embedding_cache_base::kCacheSetSize * embedding_dim_in_int4;
  ...
  using Updater                  = wholegraph_tensor_ops::CacheSetUpdater<IndexT>;
  Updater updater;
  ...
  cudaStream_t stream)
{
  ...
  wholegraph_handle_t wholegraph_handle =
    wholegraph_tensor_get_memory_handle(wg_raw_memory_embedding);
  ...
  auto* raw_embedding_desc =
    wholegraph_tensor_get_tensor_description(wholegraph_tensor_get_root(wg_raw_memory_embedding));
  size_t embedding_entry_start = 0;
  WHOLEGRAPH_RETURN_ON_FAIL(
    wholegraph_tensor_get_local_entry_start(&embedding_entry_start, wg_raw_memory_embedding));
  ...
  try {
    ...
    WHOLEGRAPH_ERROR("SortUniqueLocalIndicesTempFunc failed.");
  ...
  WHOLEGRAPH_RETURN_ON_FAIL(
    wholegraph_get_local_memory(&embedding_local_pointer,
                                 nullptr,
                                 nullptr,
                                 wholegraph_tensor_get_memory_handle(wg_raw_memory_embedding)));
  int const embedding_dim = raw_embedding_desc->strides[0];
  size_t const dtype_size = wholegraph_dtype_get_element_size(raw_embedding_desc->dtype);
  WHOLEGRAPH_CHECK_NOTHROW(embedding_dim * dtype_size % 16 == 0);
  ...
  DISPATCH_ONE_TYPE(
    ...
    static_cast<uint16_t*>(wholegraph_tensor_get_data_pointer(cache_local_data->cache_line_tag_)),
    static_cast<uint16_t*>(
      wholegraph_tensor_get_data_pointer(cache_local_data->cache_line_lfu_count_)),
    static_cast<int64_t*>(wholegraph_tensor_get_data_pointer(cache_local_data->access_count_)),
    static_cast<int4*>(wholegraph_tensor_get_data_pointer(cache_local_data->cache_line_data_)),
                                         ...
                                         int cache_set_coverage)
{
  ...
  for (int i = 0; i < need_load_count; i++) {
    ...
    output_local_write_cache_index[i] =
      cache_line_idx + cache_set_lid * wholegraph::embedding_cache_base::kCacheSetSize;
  }
  ...
  cudaStream_t stream)
{
  ...
  WHOLEGRAPH_RETURN_ON_FAIL(wholegraph_communicator_get_size(&cache_world_size, cache_comm));
  WHOLEGRAPH_RETURN_ON_FAIL(wholegraph_communicator_get_rank(&cache_world_rank, cache_comm));
  ...
  try {
    DISPATCH_ONE_TYPE(
      ...
      static_cast<const int*>(unique_count_handle.pointer()),
      static_cast<uint16_t*>(
        wholegraph_tensor_get_data_pointer(cache_local_data->cache_line_tag_)),
      static_cast<uint16_t*>(
        wholegraph_tensor_get_data_pointer(cache_local_data->cache_line_lfu_count_)),
      static_cast<int64_t*>(wholegraph_tensor_get_data_pointer(cache_local_data->access_count_)),
    ...
    WHOLEGRAPH_ERROR("DetermineLoadCacheTempFunc failed.");
    return WHOLEGRAPH_LOGIC_ERROR;
  }
  temp_memory_handle temp_cache_buffer_handle(p_env_fns);
  wholegraph_tensor_description_t temp_cache_desc =
    *wholegraph_tensor_get_tensor_description(wg_raw_memory_embedding);
  ...
  void* temp_cache_ptr           = temp_cache_buffer_handle.device_malloc(
    wholegraph_get_memory_element_count_from_tensor(&temp_cache_desc), temp_cache_desc.dtype);
  wholegraph_tensor_t temp_cache_tensor;
  WHOLEGRAPH_RETURN_ON_FAIL(
    wholegraph_make_tensor_from_pointer(&temp_cache_tensor, temp_cache_ptr, &temp_cache_desc));

  wholegraph_tensor_description_t cache_indice_desc;
  wholegraph_copy_array_desc_to_tensor(&cache_indice_desc, &indice_desc);
  cache_indice_desc.sizes[0]       = indices_num_run;
  cache_indice_desc.storage_offset = 0;

  wholegraph_tensor_t gather_indice_tensor, scatter_indice_tensor;
  ...
  WHOLEGRAPH_RETURN_ON_FAIL(wholegraph_gather(
    wg_raw_memory_embedding, gather_indice_tensor, temp_cache_tensor, p_env_fns, stream));

  WHOLEGRAPH_RETURN_ON_FAIL(wholegraph_scatter(temp_cache_tensor,
                                                 scatter_indice_tensor,
                                                 cache_local_data->cache_line_data_,
                                                 p_env_fns,
                                                 stream));

  WHOLEGRAPH_RETURN_ON_FAIL(wholegraph_destroy_tensor(temp_cache_tensor));
  WHOLEGRAPH_RETURN_ON_FAIL(wholegraph_destroy_tensor(gather_indice_tensor));
  WHOLEGRAPH_RETURN_ON_FAIL(wholegraph_destroy_tensor(scatter_indice_tensor));
  ...
  cudaStream_t stream)
{
  ...
  WHOLEGRAPH_RETURN_ON_FAIL(
    wholegraph_tensor_get_local_entry_count(&embedding_entry_count, wg_raw_memory_embedding));
  WHOLEGRAPH_CHECK_NOTHROW(embedding_entry_count % cache_set_coverage == 0);

  wholegraph_tensor_t raw_local_tensor;
  WHOLEGRAPH_RETURN_ON_FAIL(
    wholegraph_tensor_map_local_tensor(wg_raw_memory_embedding, &raw_local_tensor));
  int cache_set_count = wholegraph::div_rounding_up_unsafe(
    wholegraph_tensor_get_tensor_description(raw_local_tensor)->sizes[0], cache_set_coverage);
  ...
  if (cache_set_count > 0) {
    WriteBackCacheDirectKernel<<<cache_set_count, 32, 0, stream>>>(
      static_cast<uint16_t*>(
        wholegraph_tensor_get_data_pointer(cache_local_data->cache_line_tag_)),
      static_cast<int4*>(wholegraph_tensor_get_data_pointer(cache_local_data->cache_line_data_)),
      static_cast<int4*>(wholegraph_tensor_get_data_pointer(raw_local_tensor)),
      embedding_dim_in_int4,
      cache_set_coverage,
      drop_all);
  ...
  WHOLEGRAPH_RETURN_ON_FAIL(wholegraph_destroy_tensor(raw_local_tensor));
