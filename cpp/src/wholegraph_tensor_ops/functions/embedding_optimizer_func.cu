...
#include "wholegraph_tensor_ops/functions/embedding_cache_func.cuh"
...
void set_memory_to_float_value(float* data_ptr, float value, size_t elt_count, cudaStream_t stream)
{
  ...
  int block_count        = wholegraph::div_rounding_up_safe<int64_t>(elt_count, thread_count);
...
static void check_optimizer_inputs(wholegraph_tensor_t indices,
...
{
  WHOLEGRAPH_CHECK_NOTHROW(indices != nullptr && grads != nullptr && local_embedding != nullptr);
  if (cache_set_coverage > 0) {
    WHOLEGRAPH_CHECK_NOTHROW(local_embedding_cache_tag != nullptr &&
                              local_embedding_cache_data != nullptr);
  }
  if (PerElementCount > 0) {
    WHOLEGRAPH_CHECK_NOTHROW(per_element_local_state != nullptr);
    if (cache_set_coverage > 0) {
      WHOLEGRAPH_CHECK_NOTHROW(per_element_local_cache_tag != nullptr &&
                                per_element_local_cache_data != nullptr);
    }
  }
  auto* indices_desc = wholegraph_tensor_get_tensor_description(indices);
  WHOLEGRAPH_CHECK_NOTHROW(indices_desc->dim == 1);
  WHOLEGRAPH_CHECK_NOTHROW(indices_desc->dtype == WHOLEGRAPH_DT_INT ||
                            indices_desc->dtype == WHOLEGRAPH_DT_INT64);
  WHOLEGRAPH_CHECK_NOTHROW(indices_desc->storage_offset == 0);

  auto* grads_desc = wholegraph_tensor_get_tensor_description(grads);
  WHOLEGRAPH_CHECK_NOTHROW(grads_desc->dim == 2);
  WHOLEGRAPH_CHECK_NOTHROW(grads_desc->dtype == WHOLEGRAPH_DT_FLOAT);
  WHOLEGRAPH_CHECK_NOTHROW(grads_desc->storage_offset == 0);
  WHOLEGRAPH_CHECK_NOTHROW(grads_desc->sizes[0] == indices_desc->sizes[0]);

  int embedding_dim = grads_desc->sizes[1];

  auto* local_embedding_desc = wholegraph_tensor_get_tensor_description(local_embedding);
  WHOLEGRAPH_CHECK_NOTHROW(local_embedding_desc->dim == 2);
  auto emb_dtype = local_embedding_desc->dtype;
  WHOLEGRAPH_CHECK_NOTHROW(emb_dtype == WHOLEGRAPH_DT_FLOAT || emb_dtype == WHOLEGRAPH_DT_HALF ||
                            emb_dtype == WHOLEGRAPH_DT_BF16);
  WHOLEGRAPH_CHECK_NOTHROW(local_embedding_desc->storage_offset == 0);
  ...
  WHOLEGRAPH_CHECK_NOTHROW(wholegraph::round_up_unsafe<int>(embedding_dim, align_count) ==
                            padded_embedding_dim);

  if (cache_set_coverage > 0) {
    auto* local_embedding_cache_tag_desc =
      wholegraph_tensor_get_tensor_description(local_embedding_cache_tag);
    WHOLEGRAPH_CHECK_NOTHROW(local_embedding_cache_tag_desc->dim == 2);
    WHOLEGRAPH_CHECK_NOTHROW(local_embedding_cache_tag_desc->dtype == WHOLEGRAPH_DT_INT16);
    WHOLEGRAPH_CHECK_NOTHROW(local_embedding_cache_tag_desc->storage_offset == 0);
    WHOLEGRAPH_CHECK_NOTHROW(local_embedding_cache_tag_desc->sizes[1] == 32);
    int64_t local_cache_set_count = local_embedding_cache_tag_desc->sizes[0];
    WHOLEGRAPH_CHECK_NOTHROW(local_cache_set_count * cache_set_coverage >=
                              local_embedding_entry_count);
    auto* local_embedding_cache_data_desc =
      wholegraph_tensor_get_tensor_description(local_embedding_cache_data);
    WHOLEGRAPH_CHECK_NOTHROW(local_embedding_cache_data_desc->dim == 2);
    WHOLEGRAPH_CHECK_NOTHROW(local_embedding_cache_data_desc->dtype == emb_dtype);
    WHOLEGRAPH_CHECK_NOTHROW(local_embedding_cache_data_desc->storage_offset == 0);
    WHOLEGRAPH_CHECK_NOTHROW(local_embedding_cache_data_desc->strides[0] == padded_embedding_dim);
    WHOLEGRAPH_CHECK_NOTHROW(local_embedding_cache_data_desc->sizes[0] ==
                              local_cache_set_count * 32);
  }

  if (PerElementCount > 0) {
    auto* local_per_element_desc =
      wholegraph_tensor_get_tensor_description(per_element_local_state);
    WHOLEGRAPH_CHECK_NOTHROW(local_per_element_desc->dim == 2);
    WHOLEGRAPH_CHECK_NOTHROW(local_per_element_desc->dtype == WHOLEGRAPH_DT_FLOAT);
    WHOLEGRAPH_CHECK_NOTHROW(local_per_element_desc->storage_offset == 0);
    WHOLEGRAPH_CHECK_NOTHROW(local_per_element_desc->sizes[0] == local_embedding_entry_count);
    WHOLEGRAPH_CHECK_NOTHROW(local_per_element_desc->sizes[1] ==
    ...
    if (cache_set_coverage > 0) {
      auto* per_element_local_cache_tag_desc =
        wholegraph_tensor_get_tensor_description(per_element_local_cache_tag);
      WHOLEGRAPH_CHECK_NOTHROW(per_element_local_cache_tag_desc->dim == 2);
      WHOLEGRAPH_CHECK_NOTHROW(per_element_local_cache_tag_desc->dtype == WHOLEGRAPH_DT_INT16);
      WHOLEGRAPH_CHECK_NOTHROW(per_element_local_cache_tag_desc->storage_offset == 0);
      WHOLEGRAPH_CHECK_NOTHROW(per_element_local_cache_tag_desc->sizes[1] == 32);
      int64_t local_cache_set_count = per_element_local_cache_tag_desc->sizes[0];
      WHOLEGRAPH_CHECK_NOTHROW(local_cache_set_count * cache_set_coverage >=
                                local_embedding_entry_count);
      auto* per_element_local_cache_data_desc =
        wholegraph_tensor_get_tensor_description(per_element_local_cache_data);
      WHOLEGRAPH_CHECK_NOTHROW(per_element_local_cache_data_desc->dim == 2);
      WHOLEGRAPH_CHECK_NOTHROW(per_element_local_cache_data_desc->dtype == WHOLEGRAPH_DT_FLOAT);
      WHOLEGRAPH_CHECK_NOTHROW(per_element_local_cache_data_desc->storage_offset == 0);
      WHOLEGRAPH_CHECK_NOTHROW(per_element_local_cache_data_desc->sizes[1] ==
                                local_per_element_dim);
      WHOLEGRAPH_CHECK_NOTHROW(per_element_local_cache_data_desc->sizes[0] ==
                                local_cache_set_count * 32);
    }
  }
}
                                  ...
                                  cudaStream_t stream)
{
  ...
  int thread_count = wholegraph::div_rounding_up_unsafe(embedding_dim, 4);
  ...
  try {
    ...
    if (cache_set_coverage > 0) {
      local_embedding_cache_tag_pr =
        static_cast<uint16_t*>(wholegraph_tensor_get_data_pointer(local_embedding_cache_tag));
      local_embedding_cache_data_ptr =
        wholegraph_tensor_get_data_pointer(local_embedding_cache_data);
    }
    ...
    DISPATCH_TWO_TYPES(indice_desc->dtype,
                       ...
                       static_cast<float*>(wholegraph_tensor_get_data_pointer(grads)),
                       wholegraph_tensor_get_data_pointer(local_embedding),
                       local_embedding_cache_tag_pr,
                       local_embedding_cache_data_ptr,
                       local_entry_offset,
                       indice_desc->sizes[0],
    ...
    WHOLEGRAPH_ERROR("%s", wle.what());
    return WHOLEGRAPH_LOGIC_ERROR;
  } catch (wholegraph::cuda_error& wce) {
    WHOLEGRAPH_ERROR("%s", wce.what());
    return WHOLEGRAPH_CUDA_ERROR;
  } catch (...) {
    WHOLEGRAPH_ERROR("File %s, line %d, Unknown error", __FILE__, __LINE__);
                                                  ...
                                                  cudaStream_t stream)
{
  try {
    ...
    auto* local_embedding_desc = wholegraph_tensor_get_tensor_description(local_embedding);
    int64_t local_embedding_entry_count = local_embedding_desc->sizes[0];
    WHOLEGRAPH_CHECK_NOTHROW(per_embedding_local_state != nullptr);
    auto* per_embedding_local_state_desc =
      wholegraph_tensor_get_tensor_description(per_embedding_local_state);
    WHOLEGRAPH_CHECK_NOTHROW(per_embedding_local_state_desc->dim == 2);
    WHOLEGRAPH_CHECK_NOTHROW(per_embedding_local_state_desc->dtype == WHOLEGRAPH_DT_FLOAT);
    WHOLEGRAPH_CHECK_NOTHROW(per_embedding_local_state_desc->storage_offset == 0);
    WHOLEGRAPH_CHECK_NOTHROW(per_embedding_local_state_desc->sizes[1] == 2);
    ...
    WHOLEGRAPH_CHECK_NOTHROW(local_embedding_entry_count ==
                              per_embedding_local_state_desc->sizes[0]);
    ...
    if (cache_set_coverage > 0) {
      ...
      local_embedding_cache_data_ptr =
        wholegraph_tensor_get_data_pointer(local_embedding_cache_data);
      per_element_local_cache_tag_ptr =
        static_cast<uint16_t*>(wholegraph_tensor_get_data_pointer(per_element_local_cache_tag));
      per_element_local_cache_data_ptr =
        static_cast<float*>(wholegraph_tensor_get_data_pointer(per_element_local_cache_data));
    }
    ...
    DISPATCH_TWO_TYPES(
      ...
      local_embedding_cache_tag_pr,
      local_embedding_cache_data_ptr,
      static_cast<float*>(wholegraph_tensor_get_data_pointer(per_element_local_state)),
      per_element_local_cache_tag_ptr,
      per_element_local_cache_data_ptr,
      static_cast<float*>(wholegraph_tensor_get_data_pointer(per_embedding_local_state)),
      local_entry_offset,
      indice_desc->sizes[0],
      grads_desc->sizes[1],
      grads_desc->strides[0],
