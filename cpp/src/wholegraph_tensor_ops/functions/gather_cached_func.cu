...
__global__ void gather_cached_kernel(wholegraph_gref_t padded_embedding_gref,
                                     int stride_in_int4,
                                     ...
                                     int64_t raw_start_gid)
{
  ...
  wholegraph::device_reference<uint16_t> cache_line_tag_dev_ref(cache_line_tag_gref);
  ...
  wholegraph::device_reference<int4> embedding_dev_ref(padded_embedding_gref);
  wholegraph::device_reference<int4> cached_embedding_dev_ref(cached_embedding_gref);
...
template <typename EmbedT, typename OutputT, typename IndexT>
void gather_cached_temp_func(wholegraph_gref_t padded_embedding_gref,
                             wholegraph_matrix_description_t embedding_desc,
                             wholegraph_gref_t cached_embedding_gref,
                             wholegraph_matrix_description_t cached_embedding_desc,
                             wholegraph_gref_t cache_line_tag_gref,
                             void* input_indices,
                             ...
                             cudaStream_t stream)
{
  ...
  WHOLEGRAPH_CHECK_NOTHROW(embedding_desc.stride == cached_embedding_desc.stride);
  WHOLEGRAPH_CHECK_NOTHROW(embedding_desc.stride *
                              wholegraph_dtype_get_element_size(embedding_desc.dtype) %
                              sizeof(int4) ==
                            0);
  WHOLEGRAPH_CHECK_NOTHROW(embedding_desc.sizes[1] == output_desc.sizes[1]);
  WHOLEGRAPH_CHECK_NOTHROW(indices_desc.size == output_desc.sizes[0]);
  int stride_in_int4 =
    embedding_desc.stride * wholegraph_dtype_get_element_size(embedding_desc.dtype) / sizeof(int4);
                                            ...
                                            cudaStream_t stream)
{
  if (embedding_desc->dim != 2 || cached_embedding_desc->dim != 2 || indices_desc->dim != 1 ||
      output_desc->dim != 2) {
    WHOLEGRAPH_ERROR("tensor dim not right.");
  ...
  if (embedding_desc->strides[1] != 1 || cached_embedding_desc->strides[1] != 1 ||
    ...
    WHOLEGRAPH_ERROR("tensor stride of last dim should be 1.");
    return WHOLEGRAPH_INVALID_INPUT;
  }
  if (embedding_desc->strides[0] != cached_embedding_desc->strides[0]) {
    WHOLEGRAPH_ERROR("padded_embedding and cached_embedding should have same strides[0].");
    return WHOLEGRAPH_INVALID_VALUE;
  }
  if (embedding_desc->strides[0] * wholegraph_dtype_get_element_size(embedding_desc->dtype) %
        sizeof(int4) !=
      0) {
    WHOLEGRAPH_ERROR("embedding should be aligned to int4 (16 bytes).");
    return WHOLEGRAPH_INVALID_INPUT;
  }
  if (embedding_desc->sizes[1] != output_desc->sizes[1]) {
    WHOLEGRAPH_ERROR("embedding size for embedding and output should be same, %ld v.s. %ld.",
                      embedding_desc->sizes[1],
                      output_desc->sizes[1]);
    return WHOLEGRAPH_INVALID_INPUT;
  }
  if (indices_desc->dtype != WHOLEGRAPH_DT_INT64 && indices_desc->dtype != WHOLEGRAPH_DT_INT) {
    WHOLEGRAPH_ERROR("indices should be int64 or int32.");
    return WHOLEGRAPH_INVALID_INPUT;
  }
  if (indices_desc->sizes[0] != output_desc->sizes[0]) {
    WHOLEGRAPH_ERROR("indices size and output entry count should be the same.");
    return WHOLEGRAPH_INVALID_INPUT;
  }
  if (embedding_desc->dtype != cached_embedding_desc->dtype) {
    WHOLEGRAPH_ERROR("embedding and cache should be same type");
    return WHOLEGRAPH_INVALID_INPUT;
  }
  wholegraph_dtype_t const embedding_dtype = embedding_desc->dtype;
  wholegraph_dtype_t const output_dtype    = output_desc->dtype;
  if (wholegraph_dtype_is_floating_number(embedding_dtype) &&
        !wholegraph_dtype_is_floating_number(output_dtype) ||
      wholegraph_dtype_is_integer_number(embedding_dtype) &&
        !wholegraph_dtype_is_integer_number(output_dtype)) {
    WHOLEGRAPH_ERROR("embedding and output should be all float or all integer");
    return WHOLEGRAPH_INVALID_INPUT;
  }
  wholegraph_matrix_description_t embedding_matrix_desc, cached_embedding_matrix_desc,
    output_matrix_desc;
  WHOLEGRAPH_CHECK_NOTHROW(
    wholegraph_convert_tensor_desc_to_matrix(&embedding_matrix_desc, embedding_desc));
  WHOLEGRAPH_CHECK_NOTHROW(wholegraph_convert_tensor_desc_to_matrix(&cached_embedding_matrix_desc,
                                                                      cached_embedding_desc));
  WHOLEGRAPH_CHECK_NOTHROW(
    wholegraph_convert_tensor_desc_to_matrix(&output_matrix_desc, output_desc));
  wholegraph_array_description_t indices_array_desc;
  WHOLEGRAPH_CHECK_NOTHROW(
    wholegraph_convert_tensor_desc_to_array(&indices_array_desc, indices_desc));
  if (indices_array_desc.size == 0) return WHOLEGRAPH_SUCCESS;
...
template <typename EmbedT, typename OutputT, typename IndexT>
void try_gather_cached_temp_func(wholegraph_gref_t cached_embedding_gref,
                                 wholegraph_matrix_description_t cached_embedding_desc,
                                 wholegraph_gref_t cache_line_tag_gref,
                                 void* input_indices,
                                 wholegraph_array_description_t indices_desc,
...
{
  int indice_count = indices_desc.size;
  WHOLEGRAPH_CHECK_NOTHROW(cached_embedding_desc.stride *
                              wholegraph_dtype_get_element_size(cached_embedding_desc.dtype) %
                              sizeof(int4) ==
                            0);
  WHOLEGRAPH_CHECK_NOTHROW(cached_embedding_desc.sizes[1] == output_desc.sizes[1]);
  WHOLEGRAPH_CHECK_NOTHROW(indices_desc.size == output_desc.sizes[0]);
  ...
  cudaStream_t stream)
{
  ...
  if (cached_embedding_desc->strides[0] *
        wholegraph_dtype_get_element_size(cached_embedding_desc->dtype) % sizeof(int4) !=
      0) {
    WHOLEGRAPH_ERROR("cached_embedding_desc should be aligned to int4 (16 bytes).");
    return WHOLEGRAPH_INVALID_INPUT;
  }
  if (cached_embedding_desc->sizes[1] != output_desc->sizes[1]) {
    WHOLEGRAPH_ERROR("embedding size for embedding and output should be same.");
    return WHOLEGRAPH_INVALID_INPUT;
  }
  if (indices_desc->dtype != WHOLEGRAPH_DT_INT64 && indices_desc->dtype != WHOLEGRAPH_DT_INT) {
    WHOLEGRAPH_ERROR("indices should be int64 or int32.");
  ...
  wholegraph_dtype_t const embedding_dtype = cached_embedding_desc->dtype;
  wholegraph_dtype_t const output_dtype    = output_desc->dtype;
  if (wholegraph_dtype_is_floating_number(embedding_dtype) &&
        !wholegraph_dtype_is_floating_number(output_dtype) ||
      wholegraph_dtype_is_integer_number(embedding_dtype) &&
  ...
  wholegraph_matrix_description_t embedding_matrix_desc, cached_embedding_matrix_desc,
    output_matrix_desc;
  WHOLEGRAPH_CHECK_NOTHROW(
    wholegraph_convert_tensor_desc_to_matrix(&embedding_matrix_desc, cached_embedding_desc));
  WHOLEGRAPH_CHECK_NOTHROW(wholegraph_convert_tensor_desc_to_matrix(&cached_embedding_matrix_desc,
                                                                      cached_embedding_desc));
  WHOLEGRAPH_CHECK_NOTHROW(
    wholegraph_convert_tensor_desc_to_matrix(&output_matrix_desc, output_desc));
