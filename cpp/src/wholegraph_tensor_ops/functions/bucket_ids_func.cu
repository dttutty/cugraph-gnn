                                  ...
                                  cudaStream_t stream)
{
  ...
  int block_count     = wholegraph::div_rounding_up_unsafe(indice_desc.size, BLOCK_SIZE);
...
{
  try {
    WG_CUDA_CHECK(cudaMemsetAsync(dev_rank_id_count_ptr, 0, sizeof(int64_t) * world_size, stream));
    if (indice_desc.size == 0) { return WHOLEGRAPH_SUCCESS; }
    ...
    WHOLEGRAPH_ERROR("bucket_ids_for_ranks CUDA LOGIC Error %s\n", wce.what());
