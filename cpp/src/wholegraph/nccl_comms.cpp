...
{
  WG_CUDA_CHECK(cudaMallocHost(&host_send_buffer_, HOST_BUFFER_SIZE_PER_RANK * num_ranks_));
  WG_CUDA_CHECK(cudaMallocHost(&host_recv_buffer_, HOST_BUFFER_SIZE_PER_RANK * num_ranks_));
  WG_CUDA_CHECK(cudaMalloc(&buf_, sizeof(int)));
}

nccl_comms::~nccl_comms()
{
  WG_CUDA_CHECK_NO_THROW(cudaFreeHost(host_send_buffer_));
  WG_CUDA_CHECK_NO_THROW(cudaFreeHost(host_recv_buffer_));
  WG_CUDA_CHECK_NO_THROW(cudaFree(buf_));
}
...
#endif
    default: WHOLEGRAPH_FAIL("get_nccl_datatype_size");
  }
...
void nccl_comms::barrier() const
{
  ...
  WG_CUDA_CHECK(cudaStreamSynchronize(rmm_stream_));
}
...
wholegraph_error_code_t nccl_comms::sync_stream(cudaStream_t stream) const
{
  if (raft::comms::detail::nccl_sync_stream(nccl_comm_, stream) != raft::comms::status_t::SUCCESS) {
    return WHOLEGRAPH_COMMUNICATION_ERROR;
  }
  return WHOLEGRAPH_SUCCESS;
}

wholegraph_error_code_t nccl_comms::sync_stream() const { return sync_stream(rmm_stream_); }
