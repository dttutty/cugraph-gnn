...
inline void ClosePipes(std::vector<std::array<int, 2>>* pipes)
{
  for (size_t i = 0; i < pipes->size(); i++) {
    WHOLEGRAPH_CHECK(close(pipes->at(i)[0]) == 0);
    WHOLEGRAPH_CHECK(close(pipes->at(i)[1]) == 0);
  }
  pipes->clear();
}
  ...
  int rank, int world_size, int root, const std::vector<std::array<int, 2>>& pipes, T* data)
{
  if (rank == root) {
    for (int i = 0; i < world_size; i++) {
      ...
      if (wret != sizeof(T)) { WHOLEGRAPH_FATAL("write to pipe failed."); }
    }
  }
  auto rret = read(pipes[rank][0], data, sizeof(T));
  if (rret != sizeof(T)) { WHOLEGRAPH_FATAL("read to pipe failed."); }
}
...
inline void PipeGroupBroadcast(int rank,
                               ...
                               T* data)
{
  WHOLEGRAPH_CHECK(world_size % group_size == 0);
