                             ...
                             cudaStream_t stream)
{
  ...
  bool is_float_src =
    src_desc.dtype == WHOLEGRAPH_DT_HALF || src_desc.dtype == WHOLEGRAPH_DT_FLOAT ||
    src_desc.dtype == WHOLEGRAPH_DT_DOUBLE || src_desc.dtype == WHOLEGRAPH_DT_BF16;
  bool is_float_dst =
    dst_desc.dtype == WHOLEGRAPH_DT_HALF || dst_desc.dtype == WHOLEGRAPH_DT_FLOAT ||
    dst_desc.dtype == WHOLEGRAPH_DT_DOUBLE || dst_desc.dtype == WHOLEGRAPH_DT_BF16;
...
void device_array_type_cast(void* dst,
                            ...
                            cudaStream_t stream)
{
  wholegraph_tensor_description_t dst_tensor_desc, src_tensor_desc;
  wholegraph_copy_array_desc_to_tensor(&dst_tensor_desc, &dst_desc);
  wholegraph_copy_array_desc_to_tensor(&src_tensor_desc, &src_desc);
  wholegraph_matrix_description_t dst_matrix_desc, src_matrix_desc;
  EXPECT_TRUE(wholegraph_convert_tensor_desc_to_matrix(&dst_matrix_desc, &dst_tensor_desc));
  EXPECT_TRUE(wholegraph_convert_tensor_desc_to_matrix(&src_matrix_desc, &src_tensor_desc));
...
void device_get_expected_embedding_temp_func(void* gen_buffer,
                                             wholegraph_matrix_description_t gen_buffer_desc,
                                             void* indices,
                                   ...
                                   cudaStream_t stream)
{
  ...
  if (embedding_dtype != output_desc.dtype) {
    ...
    gen_buffer              = gen_buffer_tmh.device_malloc(
      wholegraph_get_memory_element_count_from_matrix(&gen_desc), gen_desc.dtype);
  }
...
void host_random_init_indices(void* indices,
                              ...
                              int64_t max_indices)
{
  EXPECT_TRUE(indices_desc.dtype == WHOLEGRAPH_DT_INT ||
                               ...
                               wholegraph_matrix_description_t reference_desc)
{
  ...
  size_t element_size = wholegraph_dtype_get_element_size(embedding_desc.dtype);
